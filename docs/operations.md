...
`onyx-dev-doc-deck` is centered on developer tools work in Rust. The technical target is: Build a Rust toolkit that studies doc behavior through append-only fixtures, with checkpoint recovery checks and no network dependency.
...
The score is deterministic. The current threshold is 150. Risk has a penalty of 6, latency has a penalty of 3, and weight contributes a bonus of 2.
